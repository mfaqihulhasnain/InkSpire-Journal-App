...
export async function createJournalEntry(data) {
  try {
    ...
    // Get mood data
    const mood = MOODS[data.mode.toUpperCase()];
    if (!mood) throw new Error("Invalid mood");

    // Get mood image from Pixabay
    const modeImgUrl = await getPixabayImage(data.moodQuery);
...
export async function updateJournalEntry(data) {
  try {
    ...
    let modeImgUrl = existingEntry.modeImgUrl;
    if (existingEntry.mode !== mood.id) {
      modeImgUrl = await getPixabayImage(data.moodQuery);
    }
